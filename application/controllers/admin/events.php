...
class Events extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('admin/events/event_view',$data);
	...
	public function add()
	{
        ...
        $this->load->view('themes/head_view');
		$this->load->view('admin/navbar_view', $data);
		$this->load->view('admin/events/add_view');
		$this->load->view('admin/footer_view');
		$this->load->view('themes/end_view');
	}
	...
	public function edit($id)
	{
        ...
        $this->index->checklogin();
        $data['event']        =   $this->Events_model->get($id);
        ...
        if(count($data['event']) > 0) 
        {
            ...
            $this->load->view('themes/head_view');
            $this->load->view('admin/navbar_view', $data);
            $this->load->view('admin/events/edit_view',$data);
            $this->load->view('admin/footer_view');
            $this->load->view('themes/end_view');
        }
        else
	...
	public function lists($offset = 0)
	{
        ...
        $data['lang']         = $lang;
            ...
            $config['base_url'] = base_url('admin/events/lists'); // xác định trang phân trang 
            $config['total_rows'] = $this->Events_model->count($status, $lang); // xác định tổng số record 
        ...
        $data['events']        =   $this->Events_model->get(null, $status, $lang, $config['per_page'], $offset);
        $data['pagging']       =   $this->pagination->create_links();
        $datestring        =   "%Y/%m/%d %h:%i:%s";
        $data['datestring'] =   $datestring;
        $this->load->view('themes/head_view');
		$this->load->view('admin/navbar_view', $data);
		$this->load->view('admin/events/list_view', $data);
		$this->load->view('admin/footer_view');
		$this->load->view('themes/end_view');
	}
